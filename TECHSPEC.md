...
- 부모님이 은퇴할 시기가 다가오면서 부동산들을 정리하고 일부는 금융 자산으로 바꿔서 매달 현금흐름을 창출해야 한다. 예를 들어 금융자산을 10억 보유해서 매년 3% 정도를 생활비로 사용하는 방식이다. 그러면서도 인플레이션을 헷징할 수 있어야 하며, 너무 공격적이어서 20~30%가 하락해버리면, 다른 소득이 없기 때문에 회복하기가 어렵다.
...
Agent 관리 관점에서는 Langgraph를 통해 다양한 Agent가 Graph 구조로 서로 소통하며 최종 보고서를 생성한다.
